<div class="input-group {{ $class??'' }} @if($errors->has($name))input--invalid @endif">
    ...
    <input class="input-field" type="{{ $type??'text' }}" id="{{ $id??$name }}" name="{{ $name }}" value="{{ old($name, $value) }}" placeholder="{{ $placeholder??'' }}" @if($errors->has($name))aria-errormessage="error-message-{{ $id??$name }}" aria-invalid @endif {{ $attributes??'' }}>
    @if ($errors->has($name))
